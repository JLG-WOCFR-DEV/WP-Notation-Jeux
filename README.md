...
- **Sanitisation renforcée & filtres partenaires** : les entrées injectées via `jlg_platform_breakdown_entries` seront normalisées côté template (`wp_kses_post()` + troncature) dans la prochaine itération afin de sécuriser les intégrations tiers tout en conservant les `<strong>`/`<em>` autorisés.
- **Bloc Gutenberg dédié** : le plan d’implémentation du bloc `notation-jlg/platform-breakdown` (parité éditeur/front, helper JSON, tests) est détaillé dans [`docs/platform-breakdown-block-plan.md`](plugin-notation-jeux_V4/docs/platform-breakdown-block-plan.md). Les checklists responsive associées ont été ajoutées à [`docs/responsive-testing.md`](plugin-notation-jeux_V4/docs/responsive-testing.md).
