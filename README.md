...
- **Blocs Gutenberg** :
  ...
  - `notation-jlg/express-rating` pour livrer une carte express (score, badge, CTA) avec jauge dynamique et prévisualisation fidèle dans l’éditeur, idéale pour les chapeaux, encadrés ou listes de recommandations.
- **Widgets « Derniers tests » & « Deals & disponibilités »** : activés automatiquement, ils peuvent être ajoutés depuis *Apparence > Widgets* grâce aux registres `JLG_Latest_Reviews_Widget` et `JLG_Deals_Widget`. Le widget Deals récupère les offres saisies dans la metabox, applique le tri par prix, reprend le disclaimer affilié et respecte les réglages `rel="sponsored"` définis dans l’onglet Modules.
- **Game Explorer & Score Insights** : `[jlg_game_explorer]` propose une navigation filtrable accessible (GET), un nouveau filtre « Note minimale » pour isoler les tests les mieux notés, des panneaux responsives et un focus géré ; `[jlg_score_insights]` calcule moyenne/médiane, histogramme, podium plateformes, indicateur de tendance et label de consensus (écart-type + fourchette) sur une période configurée. Le module affiche aussi une carte « Rédaction vs Lecteurs » (moyennes, votes, écart), une timeline avec sparkline accessible et une section « Points les plus cités » alimentée par les points forts/faibles. Le consensus mentionne explicitement le volume analysé (« Basé sur N tests publiés ») et expose un indice de confiance (limité/modéré/élevé) pour calquer la transparence d’IGN ou OpenCritic.
...
- **API REST / intégrations** : l’endpoint sécurisé `/jlg/v1/ratings` renvoie les reviews notées (score rédaction, votes lecteurs + histogramme, statut, plateformes) avec pagination, filtres (`platform`, `search`, `status`, `from`, `to`, `orderby`) et résumé global (segments Score Insights). L’accès passe par une authentification WordPress (nonce, cookie, Application Password, etc.). Documentation : [`docs/rest-api-ratings.md`](plugin-notation-jeux_V4/docs/rest-api-ratings.md).
- **WP-CLI « export:ratings »** : la commande `wp jlg export:ratings` (options `--status`, `--from`, `--to`, `--platform`, `--search`) génère un CSV contenant note rédaction, moyenne lecteurs, delta formaté, badge coup de cœur et plateformes pour alimenter vos rapports ou partenaires.
